...
router.get("/ownedTokens", async (req, res) => {
    ...
    for (var i=0; i<heroBalance; i++){
        tokenOfOwnerByIndexPromises.push(hero.methods.tokenOfOwnerByIndex(query.address ,i).call())
    }

    for (var i=0; i<extensionBalance; i++){
        tokenOfOwnerByIndexPromises.push(extension.methods.tokenOfOwnerByIndex(query.address ,i).call())
    }    
    ...
    for (var i=0; i<tokens.length; i++){
        var uri
        if(i<heroBalance){
            uri = heroURIPrefix + tokens[i]
        } else {
            uri = extensionURIPrefix + tokens[i]
        }
        tokenURIs.push(uri)
    }
