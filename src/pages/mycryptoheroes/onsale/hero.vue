...
<template>
  <v-layout row wrap>
    ...
    <v-btn block color="grey darken-3" @click="load" v-if="heroes.length < balance">Load More</v-btn>
  </v-layout>
</template>
...
<script>
  ...
  export default {
    ...
    async asyncData({ store, params }) {
      ...
      await store.dispatch('heroes/initial', 0)
    },
    ...
    methods: {
      // ...mapGetters('questions', ['questions'])
      load() {
        this.$store.dispatch('heroes/load', this.$store.getters['heroes/heroes'].length)
      }
